...
Odoo provides the ability to cancel multiple quotations (or sales orders) at once from the list view
of the :guilabel:`Quotations` page (the main *Sales* app dashboard).

To do so, first navigate to the :guilabel:`Quotations` page by going to :menuselection:`Sales app
--> Orders --> Quotations`, or open the *Sales* app from the main Odoo dashboard.

Once on the :guilabel:`Quotations` page, all the quotations and sales order are presented in a
default list view.
...
Upon selecting the :guilabel:`Cancel quotations` option, Odoo presents a :guilabel:`Cancel
quotations` confirmation pop-up window. To complete the cancellation, click the :guilabel:`Cancel
quotations` button. To cancel, click the :guilabel:`Discard` button.

.. note::
   Odoo provides an error pop-up message when attempting to cancel an order for an ongoing
   subscription that has an invoice.
